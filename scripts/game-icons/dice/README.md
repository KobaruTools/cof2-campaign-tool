...
`src/lib/ui/diceIcons.ts`.

Même chaîne que `scripts/game-icons/classes` (icônes de profil) et
`scripts/game-icons/derived-stats` (statistiques dérivées), pour les dés polyédriques
(d4, d6, d8, d10, d12, d20). L'affichage passe toujours par le composant commun
`<DieIcon>`.

## Correspondance

Le fichier `map.tsv` associe chaque `id` de dé (valeur de `Die`,
cf. `src/data/schema.ts`) à son fichier source (`<auteur>/<icone>.svg`)
dans le dépôt [game-icons/icons](https://github.com/game-icons/icons).

## Régénérer

```sh
cd scripts/game-icons/dice
...
cp diceIcons.ts ../../../src/lib/ui/diceIcons.ts
```
