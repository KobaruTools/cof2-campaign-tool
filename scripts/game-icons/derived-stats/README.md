# Générateur des icônes de statistiques dérivées
...
`src/lib/ui/derivedStatIcons.ts`.

Même chaîne que `scripts/game-icons/classes` (icônes de profil), pour les
statistiques dérivées (PV, Défense, Initiative, etc.). L'affichage passe
toujours par le composant commun `<DerivedStatIcon>` qui cercle l'icône.

## Correspondance

Le fichier `map.tsv` associe chaque `id` de statistique (clé de `DerivedStatId`,
cf. `src/lib/ui/derivedStats.ts`) à son fichier source (`<auteur>/<icone>.svg`)
dans le dépôt [game-icons/icons](https://github.com/game-icons/icons).

## Régénérer

```sh
cd scripts/game-icons/derived-stats
...
cp derivedStatIcons.ts ../../../src/lib/ui/derivedStatIcons.ts
```
