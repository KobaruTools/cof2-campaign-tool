...
- **Structuration (PER-65)** : le coût de base n'est PAS dupliqué dans les données — il se dérive du `rank` via `spellManaCost` (`src/lib/engine/derived.ts`). Seules les vraies dérogations verbatim porteraient un champ `manaCost` sur la capacité (`schema.ts`) : coût fixe inconditionnel différent du rang, ou `0` pour un sort réellement gratuit. À ce jour aucun sort du livre n'en a : les coûts « bizarres » rencontrés sont tous des réductions DYNAMIQUES, hors coût de base. Exemple piège : Rune de garde (forgesort, rang 5) annonce « coûte seulement 3 PM », mais c'est le rang − 2 dû à sa Concentration AUTOMATIQUE (§ 5) — son coût de base reste 5. Idem invocation majeure (rang − 2), Forme animale (2 + NC), arme élémentaire (−1 PM), action de mouvement. La Concentration (§ 5, p. 228) et le surcoût d'armure (= bonus de DEF de l'armure, p. 178) se calculent par-dessus ce coût de base, jamais dedans.
