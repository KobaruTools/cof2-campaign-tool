...
/**
 * « Mes personnages » (`/characters`) = liste plate de TOUS les personnages du
 * compte (PER-180, révision). Le personnage est l'entité première : on peut en
 * créer et en consulter sans campagne ni joueur. Chaque ligne affiche un badge de
 * campagne (nom de la campagne de rattachement, ou « Non attribué »). La gestion
 * des campagnes vit sur une page dédiée (`/campaigns`), accessible depuis l'en-tête.
 *
 * Cette page occupait l'accueil `/` jusqu'à l'arrivée de la **vitrine publique** :
 * `/` est désormais une vraie page d'accueil (consultable sans session) et la liste
 * a déménagé ici, derrière l'authentification propriétaire. Le contenu, lui, est
 * inchangé.
 *
  ...
  return (
    <>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        {/* Visiteur sans compte : la page lui est ouverte (l'app est locale d'abord),
            mais il doit savoir OÙ vivent ses personnages, et ce qu'apporte un compte.
            Avertissement non bloquant, dans l'esprit permissif de la fiche (PER-45). */}
        {isAnonymous && (
          <AppAlert
            severity="info"
            sx={{ mb: 3 }}
          ...
          >
            Vos personnages sont enregistrés dans ce navigateur uniquement. Connectez-vous
            pour les synchroniser et les retrouver sur vos autres appareils.
          </AppAlert>
        )}
