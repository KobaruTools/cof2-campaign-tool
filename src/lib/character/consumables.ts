 ...
 * s'agit d'une dose d'élixir matérialisée (nom préfixé par `elixirItemName`) ou de la
 * « Bourse de 2d6 pa » du sac de départ (p. 31 : à l'usage, on tire les pa et la bourse est
 * consommée — cf. modale `CoinPouchDialog`). Tout autre objet libre est traité comme durable.
 */
export function isConsumable(line: EquipmentLine): boolean {
  if (isCustomItem(line)) {
    return (
      line.type === 'consumable' || isElixirItemName(line.name) || line.name === COIN_POUCH_ITEM_NAME
    );
  }
