...
interface PdfViewerState {
   ...
   */
  nonce: number;
  /** Ouvre le visualiseur sur `bookId`, positionné à `page` (défaut : page 1). */
  openAt: (bookId: BookId, page?: number) => void;
...
export const usePdfViewerStore = create<PdfViewerState>((set) => ({
  ...
  page: 1,
  nonce: 0,
  openAt: (bookId, page = 1) =>
    set((s) => ({ open: true, bookId, page: Math.max(1, Math.trunc(page)), nonce: s.nonce + 1 })),
