...
/**
 * Icône « pierre tombale » (game-icons.net, `lorc/tombstone.svg`, CC BY 3.0 — voir
 * NOTICE.md). Marqueur du statut « mort » d'un personnage (PER-183) : plus parlant
 * qu'un cœur brisé. Rendue via `SvgIcon` pour se comporter comme une icône MUI
 * (prop `fontSize`, couleur héritée de `currentColor`) et s'intégrer aux mêmes
 * emplacements que les autres icônes de statut. Le markup est nettoyé (fond retiré,
 * `fill` neutralisé) selon le même procédé que `src/lib/ui/*Icons.ts`.
 */
export function TombstoneIcon(props: SvgIconProps) {
  return (
    <SvgIcon data-glossary-shot="TombstoneIcon" viewBox="0 0 512 512" {...props}>
