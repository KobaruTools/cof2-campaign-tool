...
export interface SpellManaBadgeProps {
  ...
  discount?: number;
  /**
   * PER-74 — réduction PERMANENTE et INCONDITIONNELLE de coût (voie de l'invocation majeure, p. 158 :
   * « ils bénéficient automatiquement de la Concentration ») : 2 PM de moins sur TOUT sort de la
   * voie, calculée par l'appelant (`majorSummoningManaDiscount`). Distincte de `discount` (source
   * différente, tooltip dédiée) mais soustraite au même titre, avant les surcoûts. Défaut 0.
   */
  majorSummoningDiscount?: number;
 ...
 */
export function SpellManaBadge({ feature, concentration = false, surcharge = 0, armorSurcharge, discount = 0, majorSummoningDiscount = 0, size = 30, color, tooltipEnterDelay, sx }: SpellManaBadgeProps) {
  ...
  // Rituel de combat + invocation majeure (PER-74) : réductions PERMANENTES soustraites avant les
  // surcoûts, plancher 0.
  const totalDiscount = discount + majorSummoningDiscount;
  const cost = Math.max(0, preDiscountCost - totalDiscount);
  ...
  const majorSummoningReduced = majorSummoningDiscount > 0;
  ...
  const hasDetail = inflated || reduced || majorSummoningReduced;
  ...
  const extras: string[] = [];
  if (totalDiscount > 0) extras.push(`-${totalDiscount} de réduction permanente`);
  ...
  const fullTooltip = hasDetail ? (
    <Box>
      ...
      {majorSummoningReduced && (
        <Box sx={{ mt: 1 }}>
          <TooltipLine page={158}>
            Voie de l’invocation majeure : -{majorSummoningDiscount} PM (Concentration automatique)
          </TooltipLine>
        </Box>
      )}
