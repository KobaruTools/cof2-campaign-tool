...
- **Le contenu payant « Le Compagnon » (peuples, profils, voies, règles optionnelles) ne doit JAMAIS être cité publiquement**, sous aucune forme (patchnotes, commentaires de commit visibles côté public, etc.) — le repo GitHub est **public**, et ce contenu est gaté en base précisément pour rester invisible tant que le propriétaire n'a pas explicitement débloqué la source. Les patchnotes joueur (`src/data/patchnotes.json`, généré par `scripts/patchnotes/generate.ts`) filtrent automatiquement via `scripts/patchnotes/paidContentBlocklist.ts` : enrichir cette liste dès qu'un nouveau ticket Linear « (Le Compagnon) » démarre, avant son premier commit.
